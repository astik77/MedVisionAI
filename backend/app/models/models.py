...
class User(Base):
    ...
    # Relationship: one user → many prediction records
...
class PredictionHistory(Base):
    ...
    Columns:
        image_path       — relative path where the uploaded image is stored
        prediction_result — top-class label (e.g. 'Pneumonia', 'Normal')
        confidence_score  — float 0-1 representing model confidence
        gradcam_path      — path to the saved Grad-CAM overlay image
        timestamp         — UTC time of the inference
    """
